...
PATH <- "/home/eze/Documents/papers/chrom_marks/results/EP20171221-2_Gluteraldehyde2/Csvs/"
distn <- list.files(PATH, pattern="chromclass-summary.csv")
...
if (length(distn) >= 1) {
	...
	DIST <- cbind(DIST[,columns], 
		rep(sub("_chromclass-summary.csv", "", distn[1]), nrow(DIST)))
	...
	for (i in distn[2:length(distn)]) {
		...
		rep(sub("_chromclass-summary.csv", "", i), nrow(DISTprep)))
...
pdf(file="/home/eze/Documents/papers/chrom_marks/results/chrom_class3.pdf")
ggplot(DIST, aes(x=X, y=AvNorm, color=dataset, fill=dataset)) + 
	# geom_area(alpha=0.5) +
