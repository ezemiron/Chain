    ...
    for (dir_name in dir_names){
      ...
      if (length(allfilesandpaths) > 0){
        ...
        if (length(distnsum) > 0){
          spotsum <- sub ("distn-summary", "spot-class-summary_volnorm", distnsum)
          ...
          if (length (which (allfilesandpaths == spotsum))> 0){
            ...
            dfC <- dfC[, c("spotnumn","classnumn","numofcells")]
...
#rename
        dfA <- dfA[2:nrow(dfA),]
        names(dfA) <- c("condition","Marker", "class", "Log2AvNorm","lowLogErr", "uprLogErr", "spotnumn", "classnumn", "numofcells")
