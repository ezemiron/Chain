    ...
    if ((length(chromclass ==1)&(file.size(chromclass)> 0))){
      ...
      if (max(chromclass$cell)==length(filesandpaths)){
        ...
        for (fileandpath in filesandpaths){
          ...
          df <- rbind(df, dfA)
          } else {readline(prompt = "boundary file: fileandpath is empty")}
        }

      } else {readline(prompt = "number of cells in var_name spot-class-count does not match numner of boundary files")}

    } else {readline(prompt = "no var_name spot-class-count found, or empty file")}

  } else {readline(prompt = "no var_name boundaries found")}
