...
for (dir_name in dir_names){
    ...
    if (length(allfilesandpaths) > 0){
      ...
      filesandpaths  <- grep("*C488-distn.csv",allfilesandpaths, value = TRUE)
...
#    df <- transform(df, Log2AvNorm = log2(df$AvNorm))
        ...
        savename  <- paste0(var_name,"_C488-spot-class-count.csv")
        ...
        savename2  <- paste0(var_name,"_C488-spot-class-summary.csv")
        ...
        savename3 <- paste0(var_name,"_C488-spot-class-average.csv")
