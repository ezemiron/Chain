...
for (dir_name in dir_names){
  ...
  filesandpaths <- list.files(pattern= "*_mask-bound.tif", full.names=TRUE, recursive = FALSE);
  if (length(filesandpaths) > 0){

    chromclass <- list.files(pattern= "*_mask_spot-class-count.csv", full.names=TRUE, recursive = FALSE);

    if ((length(chromclass ==1)&(length(chromclass)> 0))){
