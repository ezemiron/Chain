...
for (dir_name in dir_names){
    ...
    if (length(allfilesandpaths) > 0){
      ...
      filesandpaths  <- grep("*_mask-distn.csv",allfilesandpaths, value = TRUE)
...
#    df <- transform(df, Log2AvNorm = log2(df$AvNorm))
        ...
        savename  <- paste0(var_name,"_mask_spot-class-count.csv")
        ...
        savename2  <- paste0(var_name,"_mask_spot-class-summary.csv")
	...
	savename3 <- paste0(var_name,"_mask_spot-class-average.csv")
