...
dirchosen = getwd()
csv_file = "G1_distn-compile_volnorm.csv"
topPath1 = paste(dirchosen, csv_file, sep = "/")

setwd(dirchosen)
markerlist = c("hnRNP", "H3K36me3","H4K20me3", "SCC1", "H3K27me3","CTCF","RNAP-S2P","H3K4me3")
...
p <-ggplot(inddistnbind, aes(x=class, y=Log2AvNorm, fill=condition))+
  ...
  ## geom_errorbar(aes(ymin=Log2AvNorm, ymax=Log2AvNorm), width=.1,position = pd) +
