...
#con_names <- con_names[2:length(con_names)]
  ...
  for (topdir_name in topdir_names){
    ...
    dfE <- cbind(df,df,df,df,df,df,df,df)
    dirchosen2 <- getwd()
    ...
    for (dir_name in dir_names){
      ...
      if (length(allfilesandpaths) > 0){
        distnsum  <- grep("*_EdU_distn-summary.csv",allfilesandpaths, value = TRUE)

        if (length(distnsum) > 0){
          spotsum <- sub ("_distn-summary", "_spot-class-summary", distnsum)
	  spotavg <- sub ("_distn-summary","_spot-class-average", distnsum)
...
#rename
        ...
        savename  <- paste0(con_name,"_EdU_distn-compile.csv")
	...
	names(dfE) <- c("condition","Marker","cellvol_avg","cellvol_SD","spotnum_avg","spotnum_SD", "spotnum_vol_avg","spotnum_vol_SD")
        ...
        savename2 <- paste0(con_name,"_EdU_spot-class-compile.csv")
