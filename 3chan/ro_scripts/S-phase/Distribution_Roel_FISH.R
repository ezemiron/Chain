...
datamap<- read.csv("IF-control/IF-control_distn-full1.csv")
datamap2<- read.csv("RFISH/RFISH_distn-full1.csv")
datamap3<- read.csv("Boettiger/Boettiger_distn-full1.csv")
datamap4<- read.csv("Heard/Heard_distn-full1.csv")
...
#  scale_fill_manual(values=c("#F8766D", "#D24F44", "#A62E25", "#A62E25"))+
  #scale_y_continuous(limits=c(-80, 140),breaks=seq(-80,140,20)) +
  scale_x_discrete(breaks=seq(1,7,1))+
  #theme_bw()+
  theme(axis.line = element_line(colour = "black"),
        panel.background = element_blank())
#  stat_summary(fun.y=mean, geom="line", aes(group=datamapcomp$condition)),
#  scale_colour_manual(values = c("#F8766D", "#D24F44", "#A62E25")) 
