...
for (dir_name in dir_names){
    ...
    if (length(allfilesandpaths) > 0){
      ...
      filesandpaths  <- grep("*C594-distn.csv",allfilesandpaths, value = TRUE)
...
#    df <- transform(df, Log2AvNorm = log2(df$AvNorm))
        ...
        savename  <- paste0(var_name,"_C594-spot-class-count.csv")
        ...
        savename2  <- paste0(var_name,"_C594-spot-class-summary.csv")
        write.csv(df2,savename2);

        df3 <- aggregate(df[,2:4], list(class =df$cell),sum)
        spotnum_vol <- df3[,2]/df3[,4]
        spotnum_vol_avg <- mean(spotnum_vol)/(0.041*0.041*0.125)
        spotnum_vol_SD <- sd(spotnum_vol)/(0.041*0.041*0.125)
        ...
        savename3 <- paste0(var_name,"_C594-spot-class-average.csv")
