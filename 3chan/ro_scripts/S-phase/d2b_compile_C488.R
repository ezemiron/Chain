...
#Process all d2b markers first:
  ...
  for (dir_name in dir_names){
    ...
    if (length(allfilesandpaths) > 0){
      d2bFsum  <- grep("*_C488_abs-md2b_summary.csv",allfilesandpaths, value = TRUE)
...
setwd(dirchosen2)
savename  <- paste0(con_name,"_C488_md2b-compile.csv")
...
#  
  for (dir_name in dir_names){
    ...
    if (length(allfilesandpaths) > 0){
      d2bFsum  <- grep("*_C488_d2bnorm-summary.csv",allfilesandpaths, value = TRUE)
...
setwd(dirchosen2)
savename  <- paste0(con_name,"_C488_d2bnorm-compile.csv")
write.csv(dfA1,savename);
...
#  
  for (dir_name in dir_names){
    ...
    if (length(allfilesandpaths) > 0){
      d2bFsum  <- grep("*_C488_network-summary.csv",allfilesandpaths, value = TRUE)
...
setwd(dirchosen2)
savename  <- paste0(con_name,"_C488_network-compile.csv")
write.csv(dfA1,savename);
