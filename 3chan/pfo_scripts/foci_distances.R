...
#	compiled[centroid_file, "IQ5"] <- quantile(distance, na.rm=TRUE)[5] 
	...
	if( centroid_file %in% to_downsample ) {
		...
		cent <- cent[keep,]
 		distanceD <- matrix(nrow=nrow(cent), ncol=nrow(cent))

        	for(i in 1:nrow(cent)) {
                distanceD[i,i] <- 0
                        if(i < nrow(cent)){
                                for(j in seq(from=(i+1), to=nrow(cent))) {
                                                distanceD[i,j] = distanceD[j,i] = (
                                                (cent[i,"x"]-cent[j,"x"])^2 +
                                                (cent[i,"y"]-cent[j,"y"])^2 +
                                                (cent[i, "z"]-cent[j,"z"])^2)^(1/2)
                                }
...
savename2 <- paste0("/home/eze/Documents/papers/chrom_marks/results/", "focidistances_", marker, "downsampled_June26PM.csv")
