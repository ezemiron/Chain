...
#save two files simultaniously (distance C1-C2 and distance C2-C1) 
  ...
  fprintf (fid, ",x, y, z, volume, meanintensity,chrom_class,NN_dist_1to2, NN_idx1to2\n");
  fclose(fid);
  csvwrite(NNA_C1_fpath, [cen11 nn_dist_1to2(:) nn_idx_1to2(:)], "-append");

  fid= fopen(NNA_C2_fpath, "w");
  fprintf (fid, ",x, y, z, volume, meanintensity,chrom_class,NN_dist_2to1, NN_idx_2to1\n");
  fclose(fid);
...
#save two files simultaniously (X,Y,Z coordinate with 10 distances)
  csv_fpath1 = strrep(NNA_C1_fpath, 'C488-C594_NNA.csv', 'C488-C488_NNA.csv');
  fid= fopen(csv_fpath1, "w");
  fprintf (fid, ",x,y,z,volume,meanintensity,chrom_class,nn_dist1,nn_dist2,nn_dist3,nn_dist4,nn_dist5,nn_dist6,nn_dist7,nn_dist8,nn_dist9,nn_dist10,AVG_nn_dist\n");
  fclose(fid);
  csvwrite(csv_fpath1, [cen11 nn_dist_1to1 avg_dist_1to1], "-append");

csv_fpath2 = strrep(NNA_C1_fpath, 'C488-C594_NNA.csv', 'C594-C594_NNA.csv');
